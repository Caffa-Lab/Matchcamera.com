...
`surface` 허용값은 `home`, `body`, `lens`, `database`, `accessories`, `compare`, `builder`, `resize`, `filename`, `metadata`, `rating`, `contact`, `trust`이다. 새로운 값은 클라이언트와 Worker 양쪽의 목록, 이 사전, 검증을 함께 바꿔야 한다. 허용 목록에 있다는 사실만으로 해당 화면의 모든 동작이 계측되었다고 간주하지 않는다.
