import {loadProducts, money, productLabel, productKey, matchesSearch} from './data.js';
...
function filtered(){
  ...
  if(state.query){
    list=list.filter(p=>matchesSearch(p,state.query));
  }

  for(const [k,v] of Object.entries(state.filters)){
    if(v)list=list.filter(p=>matchesFilter(p,k,v));
  }
  ...
  if(state.sort==='name'){
    list.sort((a,b)=>productLabel(a).localeCompare(productLabel(b),'ko'));
  }else{
    list.sort((a,b)=>(b.releaseYear||0)-(a.releaseYear||0)||Number(isCurrent(b))-Number(isCurrent(a))||productLabel(a).localeCompare(productLabel(b),'ko'));
  }
  return list;
}
...
function fallbackVisual(p){
  if(type==='바디'){
    return `<span class="camera-shape"><i></i></span><small>${esc(p.manufacturer||'')}</small>`;
  }
  return `<span class="lens-shape"><i></i><b></b></span><small>${esc(p.manufacturer||'')}</small>`;
}

function productVisual(p){
  const cls=type==='바디'?'camera-visual':'lens-visual';
  const photo=p.imageSrc
    ? `<img class="product-photo" src="${esc(p.imageSrc)}" alt="${esc(productLabel(p))}" loading="lazy" decoding="async">`
    : '';
  return `<div class="product-visual ${cls} ${p.imageSrc?'has-photo':''}">
    ${photo}
    <div class="product-fallback">${fallbackVisual(p)}</div>
  </div>`;
}
...
function bindImageFallbacks(){
  document.querySelectorAll('.product-photo').forEach(img=>{
    ...
    img.addEventListener('error',()=>{
      const wrap=img.closest('.product-visual');
...
function card(p){
  ...
  const price=money(p.currentPriceKrw ?? p.currentPriceUsd);
  const priceType=p.koreaPriceType||'국내 가격';
  ...
  return `<article class="finder-card">
    ...
    <div class="finder-card-body">
      <div class="card-topline">
        ...
        <span>${esc(p.koreaSaleStatus||p.saleStatus||(isCurrent(p)?'현재 판매':'단종'))}</span>
      </div>
      ...
      <div class="card-price"><small>${esc(priceType)}</small><strong>${esc(price)}</strong></div>
...
function render(){
  ...
  $('#productCards').innerHTML=visible.length
    ?visible.map(card).join('')
    :`<div class="finder-empty">조건에 맞는 ${type}가 없습니다.<br><button id="resetFiltersInline" type="button">필터 초기화</button></div>`;
...
function resetFilters(){
  ...
  document.querySelectorAll('[data-sort]').forEach(x=>x.classList.toggle('active',x.dataset.sort==='new'));
  renderFilters();
  render();
}
