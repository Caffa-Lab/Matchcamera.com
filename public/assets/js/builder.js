import {loadProducts,loadAdapters,loadBatteries,loadManufacturerOrder,sortManufacturers,findBatteriesForBody,money,productLabel,productKey,matchesSearch,brandLogoUrl} from './data.js?v=20260831-filter-fix';
...
function renderCatalog(){const xs=filteredCatalog();$('#resultCount').textContent=xs.length.toLocaleString();const shown=xs.slice(0,MAX_RENDER);$('#catalogList').innerHTML=shown.map(p=>{const selected=state.mode==='body'?state.body?.id===p.id:state.lenses.some(x=>x.id===p.id);const c=state.mode==='lens'&&state.body?checkCompatibility(state.body,p,state.adapters):null;const sub=state.mode==='body'?`${p.cameraSystem||'-'} · ${p.sensorFormat||'-'} · ${p.mount||'-'}`:`${p.cameraSystem||'-'} · ${p.mount||'-'} · ${p.focalLength||'-'} ${p.maxAperture?`· ${p.maxAperture}`:''}`;return `<article class="catalog-item ${selected?'selected':''}">${productIcon(p)}<div class="catalog-product"><div class="catalog-product-top"><b>${esc(productLabel(p))}</b>${isSale(p)?'<span class="sale-dot">판매</span>':''}</div><div class="catalog-sub">${esc(sub)}</div><div class="catalog-meta"><span>${esc(p.modelCode||p.series||'')}</span>${c?compatBadge(c):''}</div><div class="catalog-price">${money(p.currentPriceKrw??p.currentPriceUsd)} <small>확인가</small></div></div><button class="catalog-add ${selected?'remove':''}" data-${state.mode==='body'?'body':'lens'}="${esc(p.id)}" type="button">${selected?'선택됨':state.mode==='body'?'선택':'+ 추가'}</button></article>`}).join('')||`<div class="catalog-empty"><b>검색 결과가 없습니다.</b><span>필터를 줄이거나 검색어를 바꿔보세요.</span></div>`;if(xs.length>MAX_RENDER)$('#catalogList').insertAdjacentHTML('beforeend',`<div class="catalog-more">상위 ${MAX_RENDER}개만 표시 중 · 검색/필터로 좁혀보세요.</div>`)}
function renderBodySlot(){const el=$('#bodySlot');updateBodyVisual();if(!state.body){el.innerHTML=`<button class="empty-slot" data-slot-mode="body" type="button"><b>미선택</b><span>왼쪽 목록에서 바디를 선택하세요.</span></button>`;return}el.innerHTML=`<div class="selected-slot"><b>${esc(productLabel(state.body))}</b><span>${esc(state.body.manufacturer)} · ${esc(state.body.cameraSystem||'-')} · ${esc(state.body.sensorFormat||'-')} · ${esc(state.body.mount||'렌즈 고정')}</span></div><button class="slot-remove" data-remove-body type="button" aria-label="바디 제거">×</button>`}
function renderLensSlots(){const el=$('#lensSlots');if(state.body?.cameraSystem==='일체형 카메라'){el.innerHTML=`<div class="build-row lens-row"><div class="slot-icon lens-icon"></div><div class="slot-label"><strong>렌즈</strong><span>교환 불가</span></div><div class="slot-content planned-text">이 카메라는 렌즈 고정형입니다.</div><span class="soon-badge">일체형</span></div>`;return}const chosen=state.lenses.map((p,i)=>{const c=state.body?checkCompatibility(state.body,p,state.adapters):null;return `<div class="build-row lens-row">${selectedSlotVisual(p,'lens')}<div class="slot-label"><strong>렌즈 ${i+1}</strong><span>${c?esc(c.label):'렌즈 구성'}</span></div><div class="slot-content"><div class="selected-slot"><b>${esc(productLabel(p))}</b><span>${esc(p.manufacturer)} · ${esc(p.mount||'-')} · ${esc(p.focalLength||'-')} ${c?compatBadge(c):''}</span></div><button class="slot-remove" data-remove-lens="${esc(p.id)}" type="button">×</button></div><button class="slot-select" data-slot-mode="lens" type="button">렌즈 변경</button></div>`}).join('');const idx=state.lenses.length+1;el.innerHTML=chosen+`<div class="build-row lens-row add-lens-row" data-slot-mode="lens"><div class="slot-icon lens-icon"></div><div class="slot-label"><strong>렌즈 ${idx}</strong><span>복수 렌즈 추가 가능</span></div><div class="slot-content"><button class="empty-slot" data-slot-mode="lens" type="button"><b>미선택</b><span>${state.body?'호환 렌즈를 추가하세요.':'바디 선택 후 렌즈를 추가하세요.'}</span></button></div><button class="slot-select" data-slot-mode="lens" type="button">+ 렌즈 추가</button></div>`}
...
function renderSummary(){
  ...
  $('#priceBreakdown').innerHTML=priceItems.length
    ? priceItems.map(({label,product})=>{
      ...
      return `<div class="price-breakdown-row${price===null?' is-missing':''}"><span class="price-breakdown-label">${esc(label)}</span><b title="${esc(productLabel(product))}">${esc(productLabel(product))}</b><strong>${price===null?'가격 미확인':money(price)}</strong></div>`;
    }).join('')
...
async function initializeBuilder(){
  try{
    [state.products,state.adapters,state.batteries,state.manufacturerOrder]=await Promise.all([loadProducts(),loadAdapters(),loadBatteries(),loadManufacturerOrder()]);
    hydrate();
    render();
  }catch(error){
    ...
    for(const id of ['systemFilter','manufacturerFilter','mountFilter','formatFilter','saleFilter']) $("#"+id).disabled=true;
