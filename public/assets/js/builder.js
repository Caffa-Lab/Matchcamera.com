import {equipmentPayloadKg,equipmentWeightKg,SUPPORT_KINDS,supportSort,tripodHeadCompatibility,plateHeadCompatibility,supportHead} from './support-compatibility.js?v=20260911';
import {loadProducts,loadAdapters,loadBatteries,loadManufacturerOrder,loadFilterOrder,loadFlashes,loadMemoryCards,loadTripods,loadHeads,loadPlates,sortManufacturers,publicManufacturer,findBatteriesForBody,memoryCardCompatibility,supportLoadGrade,money,productLabel,productKey,matchesSearch,brandLogoUrl} from './data.js?v=20260914-fixed-lens';
...
import {priceContext,estimatePriceSummary} from './price-context.js?v=20260915-phase1';
...
const measuredEstimates=new Set();
...
function priceContextMarkup(context,{summary=false}={}){
  const sourceLink=context.sourceUrl?`<a data-price-source href="${esc(context.sourceUrl)}" target="_blank" rel="noopener noreferrer">가격 출처 ↗</a>`:'';
  const dateText=context.date?`${context.dateLabel} ${context.date}`:'';
  const status=summary?`<span class="${context.included?'':'price-excluded'}">${esc(context.label)} · ${context.included?'합계 포함':`${context.exclusionReason} · 합계 제외`}</span>`:'';
  return `<small class="price-context-meta">${status}${dateText?`<span>${esc(dateText)}</span>`:''}${sourceLink}</small>`;
}
function measureReadyEstimate(){
  if(!state.body)return;
  const fixed=state.body.cameraSystem==='일체형 카메라';
  if(!fixed&&(!state.lenses.length||state.lenses.some(lens=>!['compatible','conditional'].includes(checkCompatibility(state.body,lens,state.adapters).level))))return;
  const signature=JSON.stringify([keyOf(state.body),...state.lenses.map(keyOf).sort()]);
  if(measuredEstimates.has(signature))return;
  measuredEstimates.add(signature);
  void trackUsage('estimate_ready','builder');
}
function hydrate(){const q=new URLSearchParams(location.search);const mode=q.get('mode');if(mode==='lens'||mode==='body')state.mode=mode;const bodyKey=q.get('body');const lensKeys=(q.get('lenses')||'').split(',').filter(Boolean);if(bodyKey)state.body=state.products.find(p=>p.id===bodyKey||productKey(p)===bodyKey)||null;state.lenses=lensKeys.map(k=>state.products.find(p=>p.id===k||productKey(p)===k)).filter(Boolean);for(const [key,rows,param] of [['battery',state.batteries,'battery'],['memory',state.memoryCards,'memory'],['flash',state.flashes,'flash'],['plate',state.plates,'plate'],['head',state.heads,'head'],['tripod',state.tripods,'tripod']])state[key]=rows.find(item=>item.id===q.get(param))||null;if(state.body?.cameraSystem==='일체형 카메라'){state.mode='body';state.lenses=[]}else if(state.body&&!mode)state.mode='lens'}
...
function flashCompatibility(flash,body){
  ...
  if(body.cameraSystem==='일체형 카메라'){
    const shoe=body.specs?.['핫슈'];
    if(shoe==='없음')return {level:'incompatible',label:'장착 불가',reason:'이 카메라는 외장 플래시용 핫슈가 없습니다.'};
    return shoe==='있음'?{level:'conditional',label:'기능 확인 필요',reason:'핫슈는 있지만 TTL/HSS 지원은 해당 카메라와 플래시의 공식 호환표에서 확인해야 합니다.'}:{level:'unknown',label:'호환 미확인',reason:'이 일체형 카메라의 핫슈 및 외장 플래시 호환 정보가 확인되지 않았습니다.'};
  }
...
function mountedPayloadKg(includeHead=false){return equipmentPayloadKg({...state,head:includeHead?state.head:null});}
...
function renderSummary(){
  ...
  const estimate=estimatePriceSummary(items);
  const weightOf=product=>{const kg=equipmentWeightKg(product);return kg===null?null:kg*1000;};
  ...
  $('#priceBreakdown').innerHTML=priceItems.length
    ? priceItems.map(({label,product})=>{
      const context=priceContext(product);
      return `<div class="price-breakdown-row${context.included?'':' is-missing'}"><span class="price-breakdown-label">${esc(label)}</span><b title="${esc(accessoryLabel(product))}">${esc(accessoryLabel(product))}</b><strong>${context.amount===null?'가격 미확인':`${context.included?'':'기록 '}${money(context.amount)}`}</strong>${priceContextMarkup(context,{summary:true})}</div>`;
    }).join('')
    : '<div class="price-breakdown-empty">제품을 선택하면 개별 가격이 표시됩니다.</div>';
  $('#totalPrice').textContent=estimate.total===null?'-':money(estimate.total);
  $('#pricedCount').textContent=`${estimate.includedCount} / ${estimate.totalCount}`;
  $('#priceNote').textContent=estimate.note;
  $('#itemCount').textContent=`${items.length}개`;
  $('#totalWeight').textContent=weighted.length?`${Math.round(weight).toLocaleString()} g${weighted.length!==items.length?' +':''}`:'-';
  ...
  if(state.tripod)results.push({name:`삼각대 · ${accessoryLabel(state.tripod)}`,c:state.body?supportLoadGrade(state.tripod.maxLoadKg,mountedPayloadKg(true),{downgrade}):{level:'unknown',label:'판정 불가',reason:'바디를 먼저 선택하세요.'}});
...
function switchMode(mode){if(mode==='lens'&&state.body?.cameraSystem==='일체형 카메라'){$('#catalogMode').value='body';toast('일체형 카메라는 교환 렌즈가 없습니다.');return}state.mode=mode;state.system='all';state.manufacturer='all';state.mount='all';state.format='all';state.lensType='all';state.focalRange='all';state.query='';$('#search').value='';render()}
...
function toggleLens(id){if(state.body?.cameraSystem==='일체형 카메라'){toast('일체형 카메라는 내장 렌즈를 사용합니다.');return}const p=state.products.find(x=>x.id===id);if(!p)return;if(state.lenses.some(x=>x.id===p.id))state.lenses=state.lenses.filter(x=>x.id!==p.id);else state.lenses=[...state.lenses,p];render()}
